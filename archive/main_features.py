            ...
            for M in M_grid:
                for alpha in alpha_grid:
                    ...
                    for exp in np.arange(nb_exp):
                        ...
                        model1 = GCN(input_dim, nb_convolutions=k, output_dim=1)
                        ### Fit a GNN to predict what Y is:
                        data.train_mask = np.random.binomial(n=1, p=0.75, size=n)
                        data.test_mask = 1-data.train_mask
                        for epoch in range(1, 101):
                            loss = model1.train_data(data, data.train_mask, criterion)
                            print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}')
            
                    #pred = model(data.x, data.edge_index)

                        Y_pred = model1(data.x, data.edge_index)
                        train_acc = np.mean(np.square(Y_pred[data.train_mask].tonumpy() - f[data.train_mask].tonumpy())) 
