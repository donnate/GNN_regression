...
then
    ...
    if [ "$4" == "Tree" ]
    then
        python3 src/simulation_tree.py --add_node $1 --noise $2 --non_linear $3 --seed ${SLURM_ARRAY_TASK_ID}
    else
        if [ "$4" == "Barbell" ]
        then
            python3 src/simulation_barbell.py --add_node $1 --noise $2 --non_linear $3 --seed ${SLURM_ARRAY_TASK_ID}
        else
            python3 src/simulation_powlaw.py --add_node $1 --noise $2 --non_linear $3 --seed ${SLURM_ARRAY_TASK_ID}
