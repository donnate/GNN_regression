...
beta_matrix = np.array([[-0.01, 0.01],[-0.1, 0.1],[-1., 1.],
                       [-2., 2],[-5., 5],
                       [-10., 10.],
                       [-15., 15.],
                       [-20., 20.],
    ...
    for scale_noise  in [args.noise]:
        for it, sparsity in enumerate(sparse_seq):
            for index_beta in np.arange(8):
                ...
                n_nodes_x = 100
                 ##### Define signal on graph
                G = nx.balanced_tree(sparsity, h[it], create_using=None)
                ...
                for L in np.arange(1,10):
                    ...
                    df_test = pd.DataFrame(results_test, columns = ["graph_type", "noise_level", "sparsity", "beta", "exp",  "smoothness1", "smoothness2",
                                                        ...
                                                        "Method", "L","fold", "Error", "Prediction",
                                                        "Bias", "Variance"])
                    df_train.to_csv("results/new_results3_non_linearity_train_simu_tree_topo_NL" + str(args.non_linear) + "_seed" +  str(args.seed)+  "_add_nodes"+str(args.add_node)  + "_prediction.csv")
                    df_test.to_csv("results/new_results3_non_linearity_test_simu_tree_topo_NL" + str(args.non_linear) + "_seed" +  str(args.seed)+  "_add_nodes"+str(args.add_node)  + "_prediction.csv")
