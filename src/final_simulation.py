...
parser.add_argument('--graph_type', required=True,  type=str) 
...
if graph_type == "latent":
    param_seq = [0., 0.05, 0.1, 0.2, 0.5, 0.75,  0.95]
elif graph_type == "barbell":
    param_seq = [4, 10, 20, 40]
...
for it_exp, exp in enumerate([args.seed]):
    ...
    for scale_noise  in [args.noise]:
        for it, param in enumerate(param_seq):
            for index_beta in np.arange(8):
                    ...
                    if prune > 0:
                        for edge in list(G.edges()):
                            if not mst.has_edge(*edge):  # If the edge is not part of the MST
                                if np.random.rand() < prune:  # With probability p, remove the edge
                                    G.remove_edge(*edge)
                else:
                    if graph_type == "barbell":  
                        G = nx.barbell_graph(param, 100 - 2 * param, create_using=None)

                    elif graph_type == "tree":
                        G = nx.balanced_tree(param, h[it], create_using=None)
                    else:
                        G = nx.powerlaw_cluster_graph(n_nodes_x, 1, param)
                ...
                results_train += [[graph_type, scale_noise,  param, beta[1],  exp, smoothness, smoothness2,smoothness_max, smoothness_cor,
                                   "Benchmark",0,  0,np.square(Y[train_index] - y_true[train_index]).mean(), 
                                    0,
                                    0,
                                    np.square(Y[train_index] - y_true[train_index]).mean() ]]
                results_test += [[graph_type, scale_noise,  param, beta[1],  exp, smoothness, smoothness2,smoothness_max, smoothness_cor,
                ...
                for L in np.arange(1,10):
                    ...
                    results_train += [[ graph_type, scale_noise,param, beta[1],  exp, smoothness, 
                    ...
                    results_test += [[ graph_type, scale_noise,param, beta[1],  exp, smoothness,
                                        smoothness2,smoothness_max, smoothness_cor, "Local Average", L, 
                    ...
                    #         results_train += [[ graph_type, scale_noise,param, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  "Graph Trend Filtering", lambda_, fold, np.square(graph_trend_predictions[train_index] - y_true[train_index]).mean(), 
                    #                         np.square(graph_trend_predictions[train_index] - Y[train_index]).mean()]]
                    #         results_test += [[ graph_type, scale_noise,param, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  "Graph Trend Filtering", lambda_, fold, np.square(graph_trend_predictions[test_index] - y_true[test_index]).mean(), 
                    ...
                    #         results_train += [[ graph_type, scale_noise,param, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  "Graph Smoothing", lambda_, fold, np.square(graph_trend_predictions[train_index] - y_true[train_index]).mean(), 
                    #                         np.square(graph_trend_predictions[train_index] - Y[train_index]).mean()]]
                    #         results_test += [[ graph_type, scale_noise,param, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  "Graph Smoothing", lambda_, fold, np.square(graph_trend_predictions[test_index] - y_true[test_index]).mean(), 
                    #                         np.square(graph_trend_predictions[test_index] - Y[test_index]).mean()]]
                    print([ graph_type, scale_noise, param, beta[1],  exp, smoothness])

                    #print(results_train)
                    df_train = pd.DataFrame(results_train, columns = ["graph_type", "noise_level", "param", "beta", "exp",  "smoothness1", "smoothness2",
                                                        "smoothness_max", "smoothness_cor",
                                                        "Method", "L","fold", "Error", "Prediction",
                                                        "Bias", "Variance"])
                    df_test = pd.DataFrame(results_test, columns = ["graph_type", "noise_level", "param", "beta", "exp",  "smoothness1", "smoothness2",
                                                        "smoothness_max", "smoothness_cor",
                                                        "Method", "L","fold", "Error", "Prediction",
                                                        "Bias", "Variance"])
                    df_train.to_csv("results/new_results2_non_linearity_train_simu_latent_topo_NL" + str(args.non_linear) + "_seed" +  str(args.seed)+  "_add_nodes"+str(args.add_node)  + graph_type  + "_prediction.csv")
                    df_test.to_csv("results/new_results2_non_linearity_test_simu_latent_topo_NL" + str(args.non_linear) + "_seed" +   str(args.seed)+  "_add_nodes"+str(args.add_node)  + graph_type  + "_prediction.csv")
