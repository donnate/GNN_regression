    ...
    for index_beta in np.arange(8):
        for exp in np.arange(20):
            ...
            df_test = pd.DataFrame(results_test, columns = ["graph_type", "sparsity", "beta", "exp",  "smoothness1", "smoothness2",
                                                ...
                                                "Method", "L","fold", "Error", "Prediction",
                                                "Bias", "Variance"])
            df_train.to_csv("~/Downloads/new_results4_train_simu_tree_topo_" + str(args.add_node) + "_prediction.csv")
            df_test.to_csv("~/Downloads/new_results4_test_simu_tree_topo_" + "_" + str(args.add_node) + "_prediction.csv")
