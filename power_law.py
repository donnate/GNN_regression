...
for sparsity in sparse_seq:
    for index_beta in np.arange(8):
        for exp in np.arange(20):
            ...
            ##### create associated graphon
            #dist = sc.spatial.distance_matrix(U, U)
            #W = 1 / (1 + np.exp(5 *dist))
            #np.fill_diagonal(W, 0)
            #A = np.random.binomial(n=1, p=W)
            G = nx.powerlaw_cluster_graph(n_nodes_x, 1, sparsity)
            ...
            train = indices[0:20]
            test = indices[21:40]
            results_train += [[  sparsity, beta[1],  exp, smoothness, smoothness2,smoothness_max, smoothness_cor, "Benchmark",0,  0,np.square(Y[train] - y_true[train]).mean(), 
                               np.square(Y[train] - y_true[train]).mean() ]]
            results_test += [[ sparsity, beta[1],  exp, smoothness, smoothness2,smoothness_max, smoothness_cor, "Benchmark",0, 0,np.square(Y[test] - y_true[test]).mean(), 
                                           0]]
            ...
            for L in np.arange(1,10):
                ...
                results_train += [[ graph_type,sparsity, beta[1],  exp, smoothness, 
                ...
                results_test += [[ graph_type,sparsity, beta[1],  exp, smoothness,
                                    smoothness2,smoothness_max, smoothness_cor, "Local Average", L, 
                ...
                results_train += [[ graph_type,sparsity, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  
            ...
            #         results_train += [[ graph_type,sparsity, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  "Graph Trend Filtering", lambda_, fold, np.square(graph_trend_predictions[train] - y_true[train]).mean(), 
            #                         np.square(graph_trend_predictions[train] - Y[train]).mean()]]
            #         results_test += [[ graph_type,sparsity, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  "Graph Trend Filtering", lambda_, fold, np.square(graph_trend_predictions[test] - y_true[test]).mean(), 
            ...
            #         results_train += [[ graph_type,sparsity, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  "Graph Smoothing", lambda_, fold, np.square(graph_trend_predictions[train] - y_true[train]).mean(), 
            #                         np.square(graph_trend_predictions[train] - Y[train]).mean()]]
            #         results_test += [[ graph_type,sparsity, beta[1],  exp, smoothness,smoothness2,smoothness_max, smoothness_cor,  "Graph Smoothing", lambda_, fold, np.square(graph_trend_predictions[test] - y_true[test]).mean(), 
            #                         np.square(graph_trend_predictions[test] - Y[test]).mean()]]
            print([ graph_type, sparsity, beta[1],  exp, smoothness])
            ...
            df_test = pd.DataFrame(results_test, columns = ["graph_type", "sparsity", "beta", "exp",  "smoothness1", "smoothness2",
                                                ...
                                                "Method", "L","fold", "Error", "Prediction",
                                                "Bias", "Variance"])
            df_train.to_csv("~/Downloads/new_results4_train_simu_PL_topo_" + str(args.add_node) + "_prediction.csv")
            df_test.to_csv("~/Downloads/new_results4_test_simu_PL_topo_" + "_" + str(args.add_node) + "_prediction.csv")
